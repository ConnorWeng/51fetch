...
crawlAll = if args[1] is '-a' then true else false
crawlItems = if crawlAll then crawlItemsInStore else crawlAllItemsInStore
...
db.query "select * from ecm_store s left join ecm_member_auth a on s.im_ww = a.vendor_user_nick and a.state = 1 where s.state = 1 and s.store_id = #{args[2]}", (err, stores) ->
  if err then throw err
  getIPProxy()
  setTimeout ->
    log "#{stores[0]['store_name']} need to be fetched."
    crawl stores[0]

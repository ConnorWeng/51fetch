...
begin
  ...
  select username into v_username from ucenter51.uc_members where username = concat('mall-', i_qq);

  if v_username is null then
    set v_username = concat('mall-', i_qq);
    set v_salt = substr(rand(), 3, 4);
    insert into ucenter51.uc_members set secques='', username=v_username, password=md5(concat(md5(v_username), v_salt)), email=concat(i_qq, '@qq.com'), regip='112.124.54.224', regdate=1443279999, salt=v_salt;

...
class db
  ...
  updateGoods: (goodsId, title, price, taobaoPrice, desc, goodHttp, realPic, skus, defaultImage, sellerCids, callback) ->
    ...
    sql += "update ecm_goods set default_image = '#{defaultImage}' where good_http = '#{goodHttp}' and (default_image = '' or default_image = 'undefined' or locate('vvic.com', default_image) != 0);"
    @query sql, (err, result) ->
      if err

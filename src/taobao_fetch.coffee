...
class taobao_fetch
  ...
  fetchUrl: (url, store) ->
    @requestHtmlContent url, (err, content) =>
      if not err
        @getJQueryFromContent content, (err, $) =>
          if err
            return console.error 'error in getJQueryFromContent ' + err
          ...
          pageUrl = @nextPage $
          if pageUrl isnt null then @fetchUrl pageUrl, store
      else
        console.error "error in fetchUrl: #{url} " + err

  getJQueryFromContent: (content, callback) ->
